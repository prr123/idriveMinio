...
func main() {
	...
	useStr := "getObjStat [/obj=objname] [/bucket=bucket] [/dbg]"
	helpStr := "program that retrieves the metadata of an object from a bucket\n  requires: /obj and /bucket flags\n"
...
//	minioClient.TraceOn(os.Stderr)
	...
	if dbg {log.Printf("getting stats from object %s from bucket %s\n", objNam, buckNam)}

	objInfo, err := minioClient.StatObject(ctx, buckNam, objNam, statOpts)
	if err != nil { log.Fatalf("error getStatObject: %v", err)}

//	if dbg {fmt.Println(objInfo)}
	minioLib.PrintStatInfo(&objInfo)
